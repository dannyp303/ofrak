...
class TestFileDataService:
    async def test_get_root_by_id(self, populated_data_service: FileDataService):
        with pytest.raises(NotFoundError):
            populated_data_service._get_root_by_id(DATA_1)
...
class TestFileDataRoot:
    ...
    def data_root(self):
        ...
        tempdir = os.path.join(tempfile._get_default_tempdir(), "test")
        if os.path.exists(tempdir):
            shutil.rmtree(tempdir)
        os.mkdir(tempdir)
        return _FileDataRoot(model, b"\xed" * 0x100, tempdir)
